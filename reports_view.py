...
def ReportsView(page: ft.Page):
        ...
        if e.path:
            try:
                if export_type == "EXCEL":
                    ...
                    save_pdf(e.path)
                page.snack_bar = ft.SnackBar(ft.Text(f"Arquivo exportado com sucesso: {e.path}"), bgcolor=ft.Colors.GREEN_700)
            except Exception as ex:
                page.snack_bar = ft.SnackBar(ft.Text(f"Erro ao exportar arquivo: {str(ex)}"), bgcolor=ft.Colors.RED_700)
            page.snack_bar.open = True
            page.update()

    # file_picker = ft.FilePicker(on_result=on_file_result)
    # page.overlay.extend([date_picker_start, date_picker_end, file_picker])
    # file_picker logic removed in favor of direct download

    def save_excel(filename):
        try:
            export_path = os.path.join(utils.get_exports_dir(), filename)
            filters = get_common_filters()
            # Busca as movimentações (aumentei o limite para o relatório)
            ...
            df.to_excel(export_path, index=False, engine='openpyxl') # Força engine openpyxl
            return filename
        except Exception as e:
            raise Exception(f"Erro no Excel: {str(e)}")
        ...
        try:
            ...
            for m in movs:
                table_data.append([
                    ...
                    m["tipo"],
                    ds.get_item_name(m["item_id"])[:20], # Limita texto para caber
                    str(m["quantidade"]),
    ...
    def export_excel_click(e):
        page.snack_bar = ft.SnackBar(ft.Text("Gerando Excel..."), bgcolor=ft.Colors.BLUE_700)
        page.snack_bar.open = True
        page.update()
        try:
            ...
            # Add cache buster to ensure browser downloads new version
            page.launch_url(f"{download_url}?_={int(datetime.now().timestamp())}", web_window_name="_blank")
            page.snack_bar = ft.SnackBar(ft.Text("Download iniciado!"), bgcolor=ft.Colors.GREEN_700)
        except Exception as ex:
            page.snack_bar = ft.SnackBar(ft.Text(str(ex)), bgcolor=ft.Colors.RED_700)
        page.snack_bar.open = True
        page.update()

    def export_pdf_click(e):
        page.snack_bar = ft.SnackBar(ft.Text("Gerando PDF..."), bgcolor=ft.Colors.BLUE_700)
        page.snack_bar.open = True
        page.update()
        try:
            # Clean timestamp for filename
            timestamp = datetime.now().strftime("%Y%m%d_%H%M")
            filename = f"Relatorio_Inventario_{timestamp}.pdf"
            save_pdf(filename)
            download_url = utils.get_download_url(filename)
            # Add cache buster to ensure browser downloads new version
            page.launch_url(f"{download_url}?_={int(datetime.now().timestamp())}", web_window_name="_blank")
    ...
    def generate_inventory_report(apt_id):
        ...
        rows = []
        for item in ds.items:
            balance = ds.get_balance(item["id"], apt_id)
            if balance > 0:
                rows.append(
                    ft.DataRow(cells=[
                        ft.DataCell(ft.Text(item["nome"])),
                        ft.DataCell(ft.Text(item["categoria"])),
                        ft.DataCell(ft.Text(str(balance), weight=ft.FontWeight.BOLD))
                    ])
                )
        return rows
    ...
    def build_report(e):
        ...
        # Inventário por Apartamento (Se apt selecionado)
        if apt_dropdown.value != "0":
            apt_name = ds.get_location_name(int(apt_dropdown.value))
            report_container.controls.append(ft.Text(f"Inventário em {apt_name}", size=18, weight="bold"))
            
            report_container.controls.append(
                ft.Container(
                    content=ft.Row([
                        ft.Text("ITEM", size=11, weight="bold", color=ft.Colors.GREY_500, expand=True),
                        ft.Text("CATEGORIA", size=11, weight="bold", color=ft.Colors.GREY_500, expand=True),
                        ft.Text("QTDE", size=11, weight="bold", color=ft.Colors.GREY_500, width=80, text_align=ft.TextAlign.CENTER),
                        ft.Text("", size=11, width=80), 
            ...
            found_items = False
            for item in ds.items:
                balance = ds.get_balance(item["id"], int(apt_dropdown.value))
                if balance > 0:
                    report_container.controls.append(build_inventory_card(item["nome"], item["categoria"], balance))
                    found_items = True

            if not found_items:
                report_container.controls.append(ft.Text("Nenhum item em estoque.", italic=True, color=ft.Colors.GREY_500))
            
            report_container.controls.append(ft.Divider(height=30))

        # Relatório de Perdas (Card Based)
