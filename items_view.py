...
def ItemsView(page: ft.Page):
    ...
    total_items_text = ft.Text("0", size=30, weight=ft.FontWeight.BOLD)
    below_min_text = ft.Text("0", size=30, weight=ft.FontWeight.BOLD, color=ft.Colors.RED_700)
    ...
    def open_item_modal(item=None):
        ...
        # Lock name and category if editing
        name_field.disabled = bool(item)
        category_dropdown.disabled = bool(item)
    ...
    def render_view(load_more=False):
        ...
        batch_items = ds.get_items(limit=ITEMS_LIMIT, offset=current_offset, filters=filters)
        ...
        if len(batch_items) == ITEMS_LIMIT:
            current_offset += ITEMS_LIMIT
    ...
    return ft.Container(
        content=ft.Column([
            ...
            summary_row,
            ft.Divider(),
            ft.Row([search_field, filter_category], spacing=10),
            ft.Column([
                grid_row, 
